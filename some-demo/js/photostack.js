...
(function(window) {
    ...
    var touchOrClick = 'ontouchstart' in window ? true : false;
    var fingerStart = touchOrClick ? 'touchstart' : 'mousedown';
    var fingerMove = touchOrClick ? 'touchmove' : 'mousemove';
    var fingerEnd = touchOrClick ? 'touchend' : 'mouseup';
    ...
    function shuffleMArray(marray) {
        var arr = [],
            marrlen = marray.length,
            inArrLen = marray[0].length;
    ...
    Photostack.prototype.attachFingerEvent = function(ele) {
        var self = this;
        var startX, startY;
        var endX, endY;
        var distanceX, distanceY;
        var isFigureMove = false;
        ele.addEventListener(fingerStart, fingerStartHandler, false);
        function fingerStartHandler(e) {
            if (!self.started) {
                return;
            }
            e.preventDefault();
        ...
        ele.addEventListener(fingerMove, fingerMoveHandler, false);
        ...
        ele.addEventListener(fingerEnd, fingerEndHandler, false);
        self.isRotating = false;
        function fingerEndHandler(e) {
            if (!self.started) {
                return;
            }
            if (!isFigureMove) {
            ...
            if ((distanceX > 20 || distanceY > 20) && !(self.isRotating)) {
                self._goPrev();
            } else if ((distanceX < -20 || distanceY < -20) && !(self.isRotating)) {
                self._goNext();
            } else {
                self._rotateItem();
            }
    ...
    Photostack.prototype._initEvents = function() {
        ...
        var self = this,
            ...
            open = function() {
                ...
                if (beforeStep) {
                    this.removeEventListener(fingerEnd, open);
        ...
        if (beforeStep) {
            this._shuffle();
            this.el.addEventListener(fingerEnd, open);
        } else {
            open();
        }

        // this.navDots.forEach(function(dot, idx) {
        //     dot.addEventListener(fingerEnd, function() {
        //         console.log(idx);
        ...
        self.attachFingerEvent(self.el);
        ...
        // next.addEventListener(fingerEnd, function() {
        //     self._goNext();
        // });
        // prev.addEventListener(fingerEnd, function() {
        //     self._goPrev();
        // });
    }

    Photostack.prototype._goNext = function() {
        var self = this;
        var goTo = self.current + 1;
        if (goTo === self.itemsCount) {
            goTo = 0;
        }

        var callback = function() {
            self._showPhoto(goTo);
        }
        if (self.flipped) {
            self._rotateItem(callback);
        } else {
    ...
    Photostack.prototype._goPrev = function() {
        var self = this;
        var goTo = self.current - 1;
        if (goTo < 0) {
            goTo = self.itemsCount - 1;
        }
    ...
    Photostack.prototype._showPhoto = function(pos) {
        ...
        // this.create.style.backgroundColor = this.colorList[this.currentItem.getAttribute('data-color')];
    ...
    Photostack.prototype._rotateItem = function(callback) {
        if (classie.hasClass(this.el, 'photostack-perspective') && !this.isRotating && !this.isShuffling) {
            ...
            // alert(this.currentItem.querySelector('.photostack-back').offsetHeight + ' ' + this.currentItem.querySelector('.photostack-back').offsetWidth + ' ' + document.querySelector('html').className + navigator.userAgent)
