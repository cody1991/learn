                ...
                var Cropit = (function() {
                    ...
                    _createClass(Cropit, [{
                        ...
                        value: function init() {
                            ...
                            this.posterListWrapper = $('.poster-list-wrapper');
                            this.posterList = $('.poster-list');
                            this.posterIndex = 0;
                            var finger = 'ontouchstart' in window ? 'touchstart' : 'mousedown';
                            var posterListW;
                            for (var i = 0; i < this.posters.colorList.length; i++) {
                                var curLi = $('<li><span></span></li>');
                                var curSpan = curLi.find('span');
                                var curLiW = Math.floor(this.posterListWrapper.width() / 6);

                                curLi.attr('data-index', i);
                                curLi.css({
                                    'width': curLiW,
                                    'height': curLiW,
                                });

                                curSpan.on(finger, function(e) {
                                    e.stopPropagation();
                                    var index = $(e.target).data('index');
                                    _this.posterIndex = index;
                                    updatePoster();
                                });

                                curLi.on(finger, function(e) {
                                    var index = $(e.target).data('index');
                                    _this.posterIndex = index;
                                    updatePoster();
                                });

                                curSpan.attr('data-index', i);
                                curSpan.css({
                                    'width': curLiW / 2,
                                    'height': curLiW / 2,
                                    'margin-left': -(curLiW / 4),
                                    'margin-top': -(curLiW / 4)
                                });
                                if (i !== this.posterIndex) {
                                    curSpan.css({
                                        'border': curLiW / 10 + 'px solid rgba(0,0,0,0.3)'
                                    })
                                }
                                posterListW = curLiW * 6;
                                curSpan.css({
                                    'background-color': this.posters.colorList[i]
                                });
                                ...
                                this.posterList.append(curLi);
                            }

                            this.posterList.width(posterListW);
                            this.posterImg = new Image();
                            this.posterImg.crossOrigin = "*";

                            this.posterImgEle = $('#poster-img');

                            updatePoster();


                            function updatePoster() {
                                var spanList = _this.posterList.find('span');
                                ...
                                spanList.eq(_this.posterIndex).css({
                                    'border-width': 0
                                });

                                if (_this.sex === 'male') {
                                    _this.posterImgEle.attr('src', _this.posters.male[_this.posterIndex]);
                                    _this.posterImg.src = _this.posters.male[_this.posterIndex];
                                } else if (_this.sex === 'female') {
                                    _this.posterImgEle.attr('src', _this.posters.female[_this.posterIndex]);
                                    _this.posterImg.src = _this.posters.female[_this.posterIndex];
                                }
                        ...
                        value: function getCroppedImageData(exportOptions) {
                            ...
                            canvasContext.drawImage(this.posterImg, 0, 0, this.posterImg.width, this.posterImg.height, 0, 0, canvas.width, canvas.height);
