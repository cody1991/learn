      ...
      <div class="list" v-if="pinyin === item.pinyin">
        <div class="shop" v-for="shop in list" v-if="shop.district.pinyin === item.pinyin">
          <a :href='"http://m.maizuo.com/v4/?co=maizuo#!/film/" + _id + "/cinema"'>
            <div class="desc">
              <div class="title">
                {{shop.name}} <span>座</span><span v-if="shop.itemTypes.length>3">通</span>
              </div>
              <div class="tip">
                <span v-for="tip in shop.labels" v-if="tip.type !== 'REFUNDABLE_SEAT_TICKET'" :class="tip.type.toLocaleLowerCase()">
                  {{tip.name}}
                </span>
              </div>
